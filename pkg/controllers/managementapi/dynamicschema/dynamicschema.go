...
var nonClusterFields = map[string]bool{"credentialconfig": true, "nodeconfig": true, "cluster": true, "nodetemplateconfig": true}
...
func (c *Controller) add(dynamicSchema *v3.DynamicSchema, key string) error {
	...
	clusterCRD, err := c.CRDs.ApiextensionsV1beta1().CustomResourceDefinitions().Get(context.Background(), "clusters.management.cattle.io", metav1.GetOptions{})
	if err != nil {
		return err
	}
	...
	_, err = c.CRDs.ApiextensionsV1beta1().CustomResourceDefinitions().Update(context.Background(), clusterCRD, metav1.UpdateOptions{})
	return err
}
